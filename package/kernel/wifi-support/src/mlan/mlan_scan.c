	...
	for (region_idx = 0;
		...
		for (next_chan = 0;
			...
			if (puser_scan_in &&
				...
				/* The below condition reduces the
				   scan_duration per channel as per parameters
				   received from event 0x76 for Coex case */
	...
	for (region_idx = 0;
		...
		for (next_chan = 0;
			...
			if (pbg_scan_in && pbg_scan_in->chan_list[0].scan_time) {
				scan_dur = pbg_scan_in->chan_list[0].scan_time;
			} else {
				if (scan_type == MLAN_SCAN_TYPE_PASSIVE)
					scan_dur = pmadapter->passive_scan_time;
				else
