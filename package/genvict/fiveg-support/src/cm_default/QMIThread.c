...
static USHORT WdsSetQMUXBindMuxDataPort(PQMUX_MSG pMUXMsg, void *arg) {
    ...
    pMUXMsg->BindMuxDataPortReq.MuxId = qmap_settings->mux_id;
...
void QmiThreadRecvQMI(PQCQMIMSG pResponse) {
    ...
    if (s_pRequest && is_response(s_pRequest, pResponse)) {
        ...
        qmidevice_send_event_to_main(RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED);
    } else if ((pResponse->QMIHdr.QMIType == QMUX_TYPE_WDA)
                    && (le16_to_cpu(pResponse->MUXMsg.QMUXMsgHdrResp.Type) == QMI_WDA_SET_LOOPBACK_CONFIG_IND)) {
                        qmidevice_send_event_to_main(RIL_UNSOL_LOOPBACK_CONFIG_IND);
        qmidevice_send_event_to_main(RIL_UNSOL_LOOPBACK_CONFIG_IND);
    } else {
        if (debug_qmi)
...
int requestSetEthMode(PROFILE_T *profile) {
    ...
    if (profile->qmap_mode) {
        s_9x07 = profile->rawIP = 1;
        qmap_settings.mux_id = profile->muxid;

        if (strncmp(profile->qmichannel, "/dev/mhi_", strlen("/dev/mhi_")) == 0) {
            qmap_settings.rx_urb_size = profile->qmap_size; //SDX24&SDX55 support 32KB 
            qmap_settings.ep_type = DATA_EP_TYPE_PCIE_V01;
            qmap_settings.iface_id = 0x05;
        }
        else { // for MDM9x07&MDM9x40&SDX20 USB
            qmap_settings.rx_urb_size = profile->qmap_size; //SDX24&SDX55 support 32KB
            qmap_settings.ep_type = DATA_EP_TYPE_HSUSB_V01;
            qmap_settings.iface_id = 0x05;
        }
        ...
        if (strncmp(profile->qmichannel, "/dev/qcqmi", strlen("/dev/qcqmi")) == 0) { //GobiNet set data format in GobiNet driver
    ...
    pRequest = ComposeQMUXMsg(QMUX_TYPE_WDA, QMIWDS_ADMIN_SET_DATA_FORMAT_REQ, WdaSetDataFormat, (void *)&qmap_settings);
...
#if 1
static void convert_cdma_mcc_2_ascii_mcc( USHORT *p_mcc, USHORT mcc )
{
...
static void convert_cdma_mnc_2_ascii_mnc( USHORT *p_mnc, USHORT imsi_11_12)
{
...
const char *wwan_data_class2str(ULONG class)
{
    unsigned int i = 0;
...
int requestRegistrationState2(UCHAR *pPSAttachedState) {
    ...
    *pPSAttachedState = 0;
    ...
    while (remainingLen > 0) {
        ...
        case 0x10: // CDMA Service Status Info
            ...
            break;
        case 0x11: // HDR Service Status Info
            if (pServiceStatusInfo->SrvStatus == 0x02) {
            ...
            break;
        case 0x12: // GSM Service Status Info
            if (pServiceStatusInfo->SrvStatus == 0x02) {
            ...
            break;
        case 0x13: // WCDMA Service Status Info
            if (pServiceStatusInfo->SrvStatus == 0x02) {
                DataCapList = WWAN_DATA_CLASS_UMTS;
                DeviceClass = DEVICE_CLASS_GSM;
            }
            break;
        case 0x14: // LTE Service Status Info
            if (pServiceStatusInfo->SrvStatus == 0x02) {
        ...
        case 0x16: // HDR System Info
            ...
            if(!requestGetHomeNetwork(&cmda_mcc, &cdma_mnc,NULL, NULL) && cmda_mcc) {
                convert_cdma_mcc_2_ascii_mcc(&MobileCountryCode, cmda_mcc);
                convert_cdma_mnc_2_ascii_mnc(&MobileNetworkCode, cdma_mnc);
            }
            break;
        case 0x17: // GSM System Info
        ...
        case 0x19: // LTE System Info
            ...
            break;
        case 0x24: // TDSCDMA Service Status Info
            if (pServiceStatusInfo->SrvStatus == 0x02) {
                pDataCapStr = "TD-SCDMA";
            }
            break;            
        case 0x25: // TDSCDMA System Info
...
int requestSetLoopBackState(UCHAR loopback_state, ULONG replication_factor)
{
    ...
    pRequest = ComposeQMUXMsg(QMUX_TYPE_WDA, QMI_WDA_SET_LOOPBACK_CONFIG_REQ, WdaSetLoopBackReq, NULL);
