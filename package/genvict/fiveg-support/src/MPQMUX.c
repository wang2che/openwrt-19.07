...
#undef dbg
#define syslog(LOG_DEBUG, format, arg... ) do {if (strlen(line) < sizeof(line)) snprintf(&line[strlen(line)], sizeof(line) - strlen(line), format, ## arg);} while (0)
...
qmi_name_item(QMIWDS_GET_PROFILE_SETTINGS_RESP), //      0x002B
...
qmi_name_item(QMIWDS_GET_LTE_ATTACH_PARAMS_REQ), //      0x0085
qmi_name_item(QMIWDS_GET_LTE_ATTACH_PARAMS_RESP), //     0x0085
...
qmi_name_item(QMINAS_SET_LTE_BAND_PRIORITY_REQ), //        0x0080
qmi_name_item(QMINAS_SET_LTE_BAND_PRIORITY_RESP), //       0x0080
};
...
qmi_name_item( QMIUIM_AUTHENTICATE_REQ),//           0x0034
qmi_name_item( QMIUIM_AUTHENTICATE_RESP),//          0x0034
...
void dump_tlv(PQCQMUX_MSG_HDR pQMUXMsgHdr) {
    int TLVFind = 0;
    int i;
    //syslog(LOG_DEBUG,"QCQMUX_TLV-----------------------------------\n");
    //syslog(LOG_DEBUG,"{Type,\tLength,\tValue}\n");
    ...
    while (1) {
        ...
        {
            syslog(LOG_DEBUG,"{%02x,\t%04x,\t", TLVHdr->TLVType, le16_to_cpu(TLVHdr->TLVLength));
            for (i = 0; i < le16_to_cpu(TLVHdr->TLVLength); i++) {
                syslog(LOG_DEBUG,"%02x ", ((UCHAR *)(TLVHdr+1))[i]);
            }
            syslog(LOG_DEBUG,"}\n");
        }
    }  // while
...
void dump_ctl(PQCQMICTL_MSG_HDR CTLHdr) {
    ...
    //syslog(LOG_DEBUG,"QCQMICTL_MSG--------------------------------------------\n");
    //syslog(LOG_DEBUG,"CtlFlags:           %02x\t\t%s\n", CTLHdr->CtlFlags, QMI_NAME(qmi_ctl_CtlFlags, CTLHdr->CtlFlags));
   syslog(LOG_DEBUG,"TransactionId:      %02x\n", CTLHdr->TransactionId);
...
int dump_qmux(QMI_SERVICE_TYPE serviceType, PQCQMUX_HDR QMUXHdr) {
    ...
    syslog(LOG_DEBUG,"TransactionId:    %04x\n", le16_to_cpu(QMUXHdr->TransactionId));

    //syslog(LOG_DEBUG,"QCQMUX_MSG_HDR-----------------------------------\n");
    switch (serviceType) {
        case QMUX_TYPE_DMS:
            syslog(LOG_DEBUG,"Type:               %04x\t%s\n", le16_to_cpu(QMUXMsgHdr->Type),
...
void dump_qmi(void *dataBuffer, int dataLen)
{
    ...
    line[0] = 0;
    for (i = 0; i < dataLen; i++) {
        syslog(LOG_DEBUG,"%02x ", ((unsigned char *)dataBuffer)[i]);
    }
    syslog(LOG_INFO,"%s", line);
    line[0] = 0;

    //syslog(LOG_DEBUG,"QCQMI_HDR-----------------------------------------");
    //syslog(LOG_DEBUG,"IFType:             %02x\t\t%s", QMIHdr->IFType, QMI_NAME(qmi_IFType, QMIHdr->IFType));
    //syslog(LOG_DEBUG,"Length:             %04x", le16_to_cpu(QMIHdr->Length));
    //syslog(LOG_DEBUG,"CtlFlags:           %02x\t\t%s", QMIHdr->CtlFlags, QMI_NAME(qmi_CtlFlags, QMIHdr->CtlFlags));
    //syslog(LOG_DEBUG,"QMIType:            %02x\t\t%s", QMIHdr->QMIType, QMI_NAME(qmi_QMIType, QMIHdr->QMIType));
    //syslog(LOG_DEBUG,"ClientId:           %02x", QMIHdr->ClientId);
