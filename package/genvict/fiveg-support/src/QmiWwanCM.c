...
int QmiWwanSendQMI(PQCQMIMSG pRequest) {
    ...
    if (cdc_wdm_fd == -1) {
        syslog(LOG_INFO,"%s cdc_wdm_fd = -1", __func__);
        return -ENODEV;
    }

    if (pRequest->QMIHdr.QMIType == QMUX_TYPE_WDS_IPV6
        || pRequest->QMIHdr.QMIType == QMUX_TYPE_WDS_2
        || pRequest->QMIHdr.QMIType == QMUX_TYPE_WDS_3
       )
    ...
    if (pollfds[0].revents & POLLOUT) {
        ...
        if (ret == nwrites) {
            ...
            syslog(LOG_INFO,"%s write=%d, errno: %d (%s)", __func__, ret, errno, strerror(errno));
        }
    } else {
        syslog(LOG_INFO,"%s poll=%d, revents = 0x%x, errno: %d (%s)", __func__, ret, pollfds[0].revents, errno, strerror(errno));
    }
...
static int QmiWwanGetClientID(UCHAR QMIType) {
    ...
    if (pResponse) {
        ...
        if (!QMUXResult && !QMUXError && (QMIType == pResponse->CTLMsg.GetClientIdRsp.QMIType)) {
            switch (QMIType) {
                case QMUX_TYPE_WDS: syslog(LOG_INFO,"Get clientWDS = %d", ClientId); break;
                case QMUX_TYPE_DMS: syslog(LOG_INFO,"Get clientDMS = %d", ClientId); break;
                case QMUX_TYPE_NAS: syslog(LOG_INFO,"Get clientNAS = %d", ClientId); break;
                case QMUX_TYPE_QOS: syslog(LOG_INFO,"Get clientQOS = %d", ClientId); break;
                case QMUX_TYPE_WMS: syslog(LOG_INFO,"Get clientWMS = %d", ClientId); break;
                case QMUX_TYPE_PDS: syslog(LOG_INFO,"Get clientPDS = %d", ClientId); break;
                case QMUX_TYPE_UIM: syslog(LOG_INFO,"Get clientUIM = %d", ClientId); break;
                case QMUX_TYPE_WDS_ADMIN: syslog(LOG_INFO,"Get clientWDA = %d", ClientId);
                break;
                default: break;
            }
            return ClientId;
...
int QmiWwanInit(void) {
    ...
    #endif
    qmiclientId[QMUX_TYPE_WDS_2] = QmiWwanGetClientID(QMUX_TYPE_WDS);
    qmiclientId[QMUX_TYPE_WDS_3] = QmiWwanGetClientID(QMUX_TYPE_WDS);
...
void * QmiWwanThread(void *pData) {
    ...
    if (cdc_wdm_fd == -1) {
        syslog(LOG_INFO,"%s Failed to open %s, errno: %d (%s)", __func__, cdc_wdm, errno, strerror(errno));
    }
    ...
    while (1) {
        ...
        for (ne = 0; ne < nevents; ne++) {
            ...
            //syslog(LOG_INFO,"{%d, %x, %x}", pollfds[ne].fd, pollfds[ne].events, pollfds[ne].revents);

            if (revents & (POLLERR | POLLHUP | POLLNVAL)) {
                syslog(LOG_INFO,"%s poll err/hup/inval", __func__);
                syslog(LOG_INFO,"poll fd = %d, events = 0x%04x", fd, revents);
                if (fd == cdc_wdm_fd) {
                } else {
                }
            ...
            if (fd == cdc_wdm_fd) {
                ...
                //syslog(LOG_INFO,"%s read=%d errno: %d (%s)",  __func__, (int)nreads, errno, strerror(errno));
                if (nreads <= 0) {
                    syslog(LOG_INFO,"%s read=%d errno: %d (%s)",  __func__, (int)nreads, errno, strerror(errno));
                    break;
                }
                ...
                if (nreads != (le16_to_cpu(pResponse->QMIHdr.Length) + 1)) {
                    syslog(LOG_INFO,"%s nreads=%d,  pQCQMI->QMIHdr.Length = %d",  __func__, (int)nreads, le16_to_cpu(pResponse->QMIHdr.Length));
                    continue;
                }
